...
if (isset($_GET['id'])) {
    ...
    if ($joueur) {
        ...
        $email_candidat = $joueur['email_candidat'];
    ...
    <main class="admin-main form-page">
        <section class="panel panel-form">
            <form method="post" action="candidat_save.php" class="form-admin" enctype="multipart/form-data">
                ...
                <input type="text" name="email_candidat" required
                    value="<?= htmlspecialchars($email_candidat) ?>">

                <label>Mot de passe candidat</label>
                <input type="password" name="mdp_candidat"
                    <?= ($mode === "ajout") ? "required" : "" ?>
                    placeholder="<?= ($mode === "edition") ? "Laisser vide pour ne pas changer" : "" ?>">
